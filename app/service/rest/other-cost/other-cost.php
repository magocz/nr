...
include_once '../../business-components/other-costs/business-activity/OtherCostsBA.php';
include_once '../../business-components/other-costs/repo/OtherCostsRepo.php';
...
if ($_SESSION['login'] == null) {
    ...
    if ($method == 'GET') {
        ...
        if (is_numeric($firstParam)) {
            ...
            echo $otherCost->generateOtherCosts($firstParam);
        }
    }
    if ($method == 'POST') {
        $firstParam = array_shift($request);
        if (is_numeric($firstParam)) {
            ...
            if ($data != null) {
                if (!$otherCost->updateOtherCost($data, $firstParam)) {
    ...
    if ($method == 'PUT') {
        ...
        $data = json_decode(file_get_contents('php://input'), true);
        if ($data != null) {
            if (!$otherCost->addNewOtherCost($data)) {
                header('HTTP/1.0 500 Server error');
                exit;
            }
        }
    ...
    if ($method == 'DELETE') {
        ...
        $firstParam = array_shift($request);
        if (is_numeric($firstParam)) {
            if (!$otherCost->deleteOtherCost($firstParam)) {
                header('HTTP/1.0 500 Server error');
                exit;
            }
        }
